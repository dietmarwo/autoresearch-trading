...
This folder contains six saved strategies evaluated with the default setup:

- Five strategies were re-evaluated with `analyze_results.py`
- `strategyMinimax27fixed.py` was generated by Codex GPT-5.4 extra high outside the agentic loop and tested separately with the same equity walk-forward configuration
...
| `strategyMinimax27fixed.py` | `mfi_rsi_adx_chandelier_v6_fixed` | Equity | Corrected control version of `strategyMinimax27.py` with conservative stop execution and whole-share partial exits | 1.49M | -0.0060 |
...
### `strategyMinimax27fixed.py`

This equity strategy was generated by Codex GPT-5.4 extra high outside the
agentic loop as a corrected control version of `strategyMinimax27.py`.

- it keeps the same broad MFI/RSI/ADX/ATR trend-continuation idea
- stop decisions use the stop carried into the current bar instead of ratcheting the stop with that same bar
- stop exits no longer assume a favorable fill above the close after the market has already moved through the stop
- partial profit-taking sells whole shares only

Under the same default equity walk-forward setup, this fixed version reached
roughly `1.49M` final value with `-0.0060` log sharpe / score, was profitable
in `10/16` folds, and generated `391` total trades. That makes it useful as a
control strategy: it still compounds modestly in raw capital terms, but the
risk-adjusted edge largely disappears once the optimistic stop handling is
removed.
